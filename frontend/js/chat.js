...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function startGeneration() {
    ...
    try {
      ...
      if (msg.includes('GROK_API_KEY') || msg.includes('not configured') || msg.includes('AI service')) {
        userMsg = '⚙️ **AI service not configured.**\n\nThe server is missing the Grok API key. Please add *GROK_API_KEY* in your Railway environment variables and redeploy.';
      } else if (msg.includes('invalid or expired') || msg.includes('401')) {
        userMsg = '🔑 **Invalid API key.**\n\nThe Grok API key is incorrect. Please check *GROK_API_KEY* in Railway variables.';
      } else if (msg.includes('rate limit') || msg.includes('429')) {
        userMsg = '⏳ **Rate limit hit.**\n\nToo many requests. Please wait 1 minute and try again.';
      } else if (msg.includes('model') && msg.includes('not found')) {
        userMsg = '🤖 **AI model not found.**\n\nSet *GROK_MODEL=grok-2-1212* in Railway variables and redeploy.';
      } else if (msg.includes('timeout')) {
        userMsg = '⏱️ **Request timed out.**\n\nThe AI took too long. Please try again.';
      } else {
        userMsg = '❌ **Generation failed:** *' + msg + '*\n\nCheck Railway logs for details.';
      }

      sendBotMessage(userMsg);
      sendBotMessage('💡 Type your **business name** again below to retry from the start.');

      currentStep        = 0;
      isProcessing       = false;
      sendBtn.disabled   = false;
      chatInput.disabled = false;
      chatInput.placeholder = STEPS[0]?.placeholder || 'Type your business name...';
    }
  }
